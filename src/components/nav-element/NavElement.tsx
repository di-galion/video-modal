	...
	useEffect(() => {
		if (closeIfLoseFocus) {
			const onClickEventHandler = e => {
				if (!document.getElementById(uniqId)?.contains(e.target)) {
					setIsOpen(false)
				}
			}
			...
			return function () {
				return document.removeEventListener('scroll', onClickEventHandler)
			}
		}
	}, [closeIfLoseFocus])
	return (
			...
			{!!headerElement ? (
				...
				<div
					...
					className={cn(styles.nav_link, classNavLink, {
						...
						[styles.no_border]: variant === EnumNavElementVariant.NO_BORDER,
						[styles.selector_filled]:
							variant === EnumNavElementVariant.SELECTOR_FILLED,
						[styles.absolute]: variant === EnumNavElementVariant.ABSOLUTE,
				...
				>
					{isDropDownIcon && (
						<DropDownIcon isOpen={isOpen} dropDownIconSize={dropDownIconSize} />
					)}
					<div
					...
					>
						{icon && (
							<div className='min-w-[27px]'>
								{typeof icon === 'string' ? (
									<i color={'#cccccc'} className={styles.material_icons}>
										{icon}
									</i>
								) : (
									icon
						...
						<div
							style={stylesTitle}
							className={classNames(styles.item_title, classTitle)}
						>
							{title}
						</div>
					</div>
			...
			{children && (
				<div
					...
					className={cn(styles.hide_show_area, classHideShowArea, {
						...
						[styles.absolute_area]: variant === EnumNavElementVariant.ABSOLUTE,
